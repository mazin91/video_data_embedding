...
int dc_dct_pred[];
{
  ...
  //============================================================
  //============================================================
  int val, i, j, sign, nc, cc, run, c, I, l;
  unsigned int code;
  DCTtab *tab;
  short *bp;
  int dct[64] = {0};
  ...
  for (i=1; ; i++)
  {
    ...
    if (tab->run==64) /* end_of_block */
    {
	  ...
	  //============================================================
	  //============================================================
	  if(data_flag==0)  //Receiving the message length
	  {
		  if (Idx == 0) printf("Started extracting length\n");

		  //Searching for the last non-zero DCT element
		  for (c = 0; c < 64; c++)
			  if (dct[c] != 0) I = c;

		  //Extracting the data
		  if ((dct[I] % 2))			    //Odd dct coeffient
		  {
			  msg[Idx++] = '1'; 
		  }
		  else if ((dct[I] % 2) == 0)	//even dct coeffient
		  {
			  msg[Idx++] = '0'; 
		  }

		  //the msg length is received
		  if (Idx == MSG_LEN_FIELD)
		  {
				l = 0;

				//Mirroring the msg length bits
				for (c = MSG_LEN_FIELD - 1; c >= 0; c--)
					temp[l++] = msg[c];

				//Converting from chars to int
				for (c = 0; c < MSG_LEN_FIELD; c++)
					if (temp[c] == '1')
						msg_len += pow(2, c);
	  ...
	  else if(data_flag==1)   //Receiving the actual data
	  {
		  if (Idx == 0) printf("Started extracting data\n");
		  ...
		  else if ((dct[I] % 2) == 0)					//even
		  {
			  msg[Idx++] = '0';
		  }

		  //the whole msg is received
		  if (Idx == msg_len)
		  {
			  msg[Idx] = '\0';
			  printf("Data is %s\n", msg);
			  fprintf_s(out_file, "%s", msg);
			  data_flag = -1;
		  }
	  }
	  //------------------------------------------
