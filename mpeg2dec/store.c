...
int frame;
{
  ...
  {
    /* progressive */
    sprintf(outname,"%s%03d",Output_Picture_Filename,frame,'f'); //Tamer modified this
...
int offset,incr,height;
{
  ...
  //Tamer Shanableh - store in .yuv format
  {
  FILE* fd;
  sprintf(tmpname,"%s.yuv",outname);
  if (!(fd = fopen(tmpname,"wb")))
  {
    fprintf(stderr,"Cannot create output YUV file !!");
	exit(-1);
  }
  fwrite(src[0],1,hsize*height,fd);
  hsize>>=1;
  height>>=1;
  fwrite(src[1],1,hsize*height,fd);
  fwrite(src[2],1,hsize*height,fd);
  fclose(fd);
  }
  return;
  //~Tamer
