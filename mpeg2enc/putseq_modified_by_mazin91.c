...
#define MSG_LEN_Frame 5
#define MSG_LEN_FIELD 32

void putseq(char *msg)
{
  /* this routine assumes (N % M) == 0 */ 
  int i, j, k, f, f0, n, np, nb, sxf, syf, sxb, syb, retCount;
  int ipflag;
  FILE *fd;
  char name[256];
  ...
  char bin_len[100];
  long int decimalNumber, remainder, quotient, ii, c, cnt = 0;
  char temp[1000000];

  //============================================================
  //============================================================
  //============================================================
  //======     Mazin's modifications    ========================
  //============================================================
  //============================================================
  //============================================================
  //---------------------------------------------------------------------
  retCount = 0;
  maximum_we_can_embed = 0;
  //Getting the msg length
  ii = 0;
  while (msg[ii] != '\0')	ii++;
  //printf("ii=%d", ii);
  ...
  if (ii == 0)
  {
	  printf("No message to be embedded\n");
	  msg_len = MSG_LEN_FIELD;
	  embedding = 1;
	  for (c = 0; c <MSG_LEN_FIELD; c++)  msg[c] = '0';
  }
  else 
  {
	  msg_len = ii;
	  embedding = 1;
	  //printf("Data need to be embedded: %s\n", msg);
	  printf("Data length = %d\n", msg_len);
	  
	  //Converting the msg length to binary and merging it with msg
	  i = 1;
	  quotient = msg_len;
	  while (quotient != 0) {
		  bin_len[i++] = quotient % 2;
		  quotient = quotient / 2;
	  }
	  for (c = 0; c <= (MSG_LEN_FIELD-i); c++)    temp[cnt++] = '0';
	  for (c = (i-1); c > 0; c--)				  temp[cnt++] = bin_len[c]+'0';
	  for (c = 0; c <msg_len; c++)				  temp[cnt++] = msg[c]; 
	  temp[cnt] = '\0';
  ...
  for (i=0; i<nframes; i++)
  {
     ...
     *
     * first GOP contains N-(M-1) frames (10), 
     * all other GOPs contain N (12) frames
     */
    f0 = N*((i+(M-1))/N) - (M-1);
    ...
    if (i==0 || (i-1)%M==0)  //No putpict here
    {
      /* I or P frame */
    ...
    if (fieldpic) //Not working at all
    {
      if (!quiet)
      {
      ...
      transform(predframe,neworg,mbinfo,blocks); //DCT

	  printf("\nFirst neworg\n");
      //putpict(neworg[0], msg);
    ...
    else
    {
      ...
      transform(predframe,neworg,mbinfo,blocks);
		...
		//============================================================
		//============================================================
		//Sending msg to putpic to for embedding
		embedded_bits = 0;
		retCount = putpict(neworg[0], msg);
		printf("This frame is carrying %d bits\n\n", retCount);
		msg = msg + embedded_bits;
		msg_len -= embedded_bits;
		//retCount = 0;
		//============================================================
		//============================================================
		//============================================================
	  ...
	  //inverse quantization
      for (k=0; k<mb_height*mb_width; k++)  //inverse quantization
      {
        if (mbinfo[k].mb_type & MB_INTRA)
          for (j=0; j<block_count; j++)
            iquant_intra(blocks[k*block_count+j],blocks[k*block_count+j],
