...
int putpict(frame, msg)
unsigned char *frame;
char *msg;
{
  int i, j, k, comp, cc, I, J, c;
  int mb_type;
  int PMV[2][2][2];
  int prev_mquant;
  int cbp, MBAinc;
  ...
  for (j=0; j<mb_height2; j++)
  {
    ...
    {
		//---------------- Slice (k = 0>>395 macroblocks loop) ---------------------------------------------------------------
      if (i==0)
      {
        /* slice header (6.2.4) */
        alignbits();
      ...
      for (comp=0; comp<block_count; comp++)
      {
        ...
        {
			maximum_we_can_embed++;
			//Data embedding
			if ((embedded_bits < msg_len) && embedding)
			{
				...
				//Data embedding in the last non-zero DCT element
				p = blocks[k*block_count + comp];
				if ((*(p + I)) % 2)
				{	//Odd DCT element
					//printf("ODD DCT\n");
					if (msg[embedded_bits] == '0')
					{
						(*(p + I))++;
						if ((*(p + I)) == 0) (*(p + I)) += 2; //-1 (odd)
					}
				}
				else
				{
					if (msg[embedded_bits] == '1')
					{
						(*(p + I))++;
					}
				}
				...
				//printf("Just embeded %c in dct coeff %d\n", msg[embedded_bits], *(p + I));
				embedded_bits++;
				if (embedded_bits == msg_len)
				{
					embedding = 0;
				}
			}
