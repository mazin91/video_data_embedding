...
char *argv[];
{
  ...
  //----  Read file  -------------------------------------------
  printf("Encoder:\n");
  if (fopen_s(&inFile, argv[3], "r") == 0)
  {
	  printf("msg.txt file is read\n");
	  fseek(inFile, 0L, SEEK_SET);
	  fscanf_s(inFile, "%s", msg, _countof(msg)); 
  }
  else
	printf("Message file is not opened\n");
  //------------------------------------------------------------
  embedded_bits = 0;
  init();
  putseq(msg);
  ...
  //printf("msg_len = %d\n", msg_len-32);
  printf("Maximum we can embed = %d bits\n", maximum_we_can_embed);
